...
La probabilité qu'une espèce soit représentée par $\nu$ individus, notée ${\pi}_{s,\nu}$, suit donc une loi binomiale négative de paramètres $k$ et $\frac{1}{p+1}$ représentée sur la figure&nbsp;\@ref(fig:nbinFig).
...
L'expérience montre que l'augmentation de l'aire d'échantillonnage n'est pas la solution adaptée, puisqu'on rencontre toujours de nouvelles espèces [@Gotelli2001], même en échantillonnant des surfaces considérables (50&nbsp;ha pour la figure&nbsp;\@ref(fig:Plotkin2000), page~\@ref(Plotkin2000), au-delà du million de \si{km²} pour la figure&nbsp;\@ref(fig:Williamson2001)).
...
La figure&nbsp;\@ref(fig:Plotkin2000)b présente la valeur de $a_i$ en fonction de la surface échantillonnée. 
...
Sur des données de richesse en nombre d'espèces d'oiseaux reproducteurs dans les îles d'un lac nord-américain, le modèle s'ajuste mieux que ceux d'Arrhenius et de Gleason (figure&nbsp;\@ref(fig:Coleman1982)).
