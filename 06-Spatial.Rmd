...
```{r gammaCode, eval=FALSE, size='footnotesize', tidy=FALSE}
library("GoFKernel")
k <- 0.2
p <- 100
...
Le code R nécessaire pour réaliser la figure est:
```{r nbinCode, eval=FALSE, size='footnotesize', tidy=FALSE}
...
```{r Schulte2005b, fig.cap="Comparaison entre le modèle de Fisher et le modèle étendu dans trois parcelles forestières connues. Les modèles sont ajustés aux données observées par @Condit1996. Les courbes légendées *Equation 4* correspondent au modèle de Fisher étendu par Schulte, alors que l'*Equation 5* est le modèle intermédiaire, supposant le nombre total d'espèces infini.", echo=FALSE}
...
```{r KemptonQCode, eval=FALSE, size='footnotesize', tidy=FALSE}
...
La fonction `specaccum` de la librairie *vegan* [@Oksanen2012] permet de tracer les SAC (figure \@ref(fig:SACFig)) selon différentes méthodes, avec un intervalle de confiance obtenu par permutation de l'ordre d'ajout des données ou à partir des formules de raréfaction.
...
Le code R nécessaire pour réaliser la figure est:
```{r specaccumCode, eval=FALSE, size='footnotesize'}
data(BCI)
...
Le code R nécessaire pour réaliser la figure est:
```{r Beguinot2015Code, eval=FALSE, size='footnotesize', tidy=FALSE}
...
Le code R nécessaire pour réaliser la figure est:
```{r Beguinot2015aCode, eval=FALSE, size='footnotesize', tidy=FALSE}
ggplot() +
