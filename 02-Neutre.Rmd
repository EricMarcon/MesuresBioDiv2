...
# Mesures neutres de la diversité $\alpha$ ou $\gamma$
...
Si aucune espèce n'est observée deux fois, l'estimateur est remplacé [@Chao2004] par $${\hat{S}}_\mathit{Chao1} = {s^{n}_{\ne 0}}+{s^{n}_{1}\left(s^{n}_{1}-1\right)}/{2}.$$
...
@Chao1987 calcule une approximation de l'intervalle de confiance à 95\%: 
...
\begin{equation}
  c=e^{t^{n}_{1-{\alpha}/{2}}\sqrt{\ln\left(1+\frac{\Var\left({\hat{S}}_\mathit{Chao1}\right)}{{\left({\hat{S}}_\mathit{Chao1}-{s^{n}_{\ne 0}}\right)}^2}\right)}}.
\end{equation}
...
Une relation entre le nombre de doubletons et les données disponibles est fourni; sa résolution numérique (le code R nécessaire est fourni avec l'article) permet d'estimer $s^{n}_{2}$ et de l'injecter dans l'estimateur Chao2.
...
L'espérance du nombre d'espèces non observées est (équation&nbsp;\@ref(eq:Esnnu)) $\sum_s{(1-p_s)^n}$, qui décroît beaucoup plus rapidement que $\sum_{i}{\frac{a_i}{n^i}}$: l'hypothèse est bien fausse.
...
```{r BiaisJack1Code, eval=FALSE, size='small'}
autoplot(Ns, Distribution="lnorm")
```

```{r BiaisJack1Fig, echo=FALSE, results='hide', out.width='\\maxwidth', ref.label='BiaisJack1Code', fig.cap="Echantillon de 1000 individus tiré dans une communauté log-normale."}
```
...
Le tableau d'entrée est un `dataframe` contenant, pour chaque espèce d'arbres ($\mathit{DBH}\ge$10&nbsp;\centim), ses effectifs par carré.
...
Le terme *completeness* a été proposé par @Beck2010 pour éviter la confusion avec le taux de couverture défini par Good (vu page~\@ref(sec:Couverture)). 
...
```{r Brose2003, fig.cap="Arbre de décision du meilleur estimateur du nombre d'espèces", echo=FALSE}
knitr::include_graphics('images/Brose2003.png')
```


Dans tous les cas, les estimateurs jackknife sont les meilleurs. 
L'arbre de décision est en Figure&nbsp;\@ref(Brose2003) [@Brose2003, fig. 6]. 
...
S'ils sont incohérents (intervalle des estimations supérieur à 50\% de leur moyenne), le critère majeur est l'équitabilité (voir page~\@ref(sec:Equitabilite)). 
...
Enfin, @Beguinot2015a; @Beguinot2016 suggère d'utiliser en règle générale le jacknife 2 (mais ne traite pas les cas dans lesquels l'échantillonnage est trop faible pour justifier un ordre supérieur) tant que  le nombre de singletons est supérieur à $2-\sqrt(2) \approx 0,6$ fois le nombre de doubletons.
...
Exemple de BCI, suite du code de la page~\@ref(code:VerifJack1) : combien de nouvelles espèces seront découvertes en échantillonnant plus?
...
```{r NonObsNFig, echo=FALSE, results='hide', ref.label='NonObsN', fig.cap="Nombre de nouvelles espèces découvertes en fonction de l'effort d'échantillonnage supplémentaire (données de BCI). Seulement 7 nouvelles espèces seront observées en échantillonnant 10000 arbres supplémentaires (environ 25&nbsp;ha en plus des 50&nbsp;ha de la parcelle qui contiennent 225 espèces)."}
```
...
```{r NonObsN, eval=FALSE, tidy=FALSE}
...
L'ajustement direct du modèle aux données, sans regroupement par octaves [@Williamson2005], est également possible (Figure&nbsp;\@ref(PrestonSansOFig)):
...
```{r PrestonSansOFig, echo=FALSE, results='hide', out.width='\\textwidth', ref.label='PrestonSansO2', fig.cap="Ajustement du modèle de Preston aux données de BCI."}
```
...
La courbe d'accumulation de BCI a été calculée page~\@ref(SACFig).
...
```{r LineweaverFig, echo=FALSE, results='hide', ref.label='Lineweaver2', fig.cap="Ajustement du même modèle de Michaelis-Menten transformé selon Lineweaver et Burk."}
```

On voit assez clairement que le modèle (Figure&nbsp;\@ref(LineweaverFig)) s'ajuste mal quand il est représenté sous cette forme [@Raaijmakers1987].
...
Le nombre de taxon de chaque niveau est lié à celui du niveau précédent, ce qui est représenté par la Figure&nbsp;\@ref(Mora2011), G [@Mora2011, figure 1] sous la forme du relation linéaire entre le logarithme du logarithme du nombre de taxons et le rang (1 pour les phylums, 5 pour les genres). 
...
```{r Mora2011, fig.cap="Évolution du nombre de taxons connus en fonction du temps et valeur estimée du nombre total (ligne horizontale grise, A à E). Le nombre d'espèces connues est trop faible pour utiliser cette méthode (F). Il est obtenu par extrapolation de la relation du nombre de taxons d'un niveau à celui du niveau supérieur (G).", echo=FALSE}
...
Une discussion approfondie est donnée par @Hubbell2015: les espèces récemment apparues au sens du modèle ne sont pas détectables avant plusieurs générations, créant un décalage entre le nombre d'espèces reconnues par la taxonomie et modèle.
...
@Hurlbert1971 l'a divisé par sa valeur maximale $1-{1}/{S}$ pour obtenir une mesure d'équitabilité valide généralisée plus tard par @Mendes2008 (voir page~\@ref(sec:Mendes)).
...
L'indice de Shannon [@Shannon1948, Shannon1963], aussi appelé indice de Shannon-Weaver ou Shannon-Wiener, ou simplement *entropie* est dérivé de la théorie de l'information:
...
$\mathrm{\Psi}(\cdot)$ est la fonction digamma, et $\hat{\alpha}$ est l'estimateur de l'indice de Fisher (voir page~\@ref(eq:AlphaFisher)):
...
@Beck2010 montrent que la correction du biais est efficace, même à des niveaux de complétude de l'échantillonnage (voir page~\@ref(Brose2003)) très faibles. @Vu2007 étudient la vitesse de convergence de l'estimateur. 
...
L'indice de Shannon est concave (Figure&nbsp;\@ref(ShannonFig) donc son estimateur (\@ref(eq:Shannon)) est biaisé négativement, même sans prendre en considération les espèces non observées.

```{r ShannonFig, echo=FALSE, results='hide', message=FALSE, ref.label='ShannonCode',  fig.cap="courbe de $x\\ln x$ entre 0 et 1.)."}
```

Code de la Figure&nbsp;\@ref(fig:SARFig):
```{r ShannonCode, eval=FALSE, tidy=FALSE, size='small'}
...
```{r ShannonBiaisFig, echo=FALSE, results='hide', message=FALSE, ref.label='ShannonBiaisCode',  fig.cap="Biais de $\\hat{p}_s \\ln\\hat{p}_s$ entre 0 et 1."}
```
...
L'estimateur $\frac{1}{S}$, si $S$ est connu, est de variance nulle mais est très biaisé. 
...
Pour les autres ordres entiers de diversité, il existe une correspondance parfaite entre les deux mesures [ @Chao2014, Annexe S2,] : la connaissance de l'une suffit permet d'obtenir l'autre. Pour $k>1$, on a [@Leinster2012]

\begin{equation}
  _{k}\!S = k + \sum_{q=2}^{k}{\binom{k}{q} (-1)^{q+1} (^{q}\!D)^{1-q}}.
\end{equation}

On a vu que $^{0}\!D$ égale $_{\infty}\!D$, donc $_{\infty}\!S=^{0}\!D$ (équation&nbsp;\@ref(eq:HurlbertD)).
...
$\mathbf{p}=(p_1,p_2,\dots,p_S)$ est le vecteur composé des probabilités d'obtenir chaque résultat. 
...
$\sum^S_{s=1}{p_s I(p_s) = H(\mathbf{p})}$. 
...
Si $\mathbf{p}$ est la distribution des probabilité des espèces dans une communauté, @Patil1982 montrent que:
  * Si $I\left(p_s\right) = \frac{1-p_s}{p_s}$, alors $H\left(\mathbf{p}\right)$ est le nombre d'espèces $S$ moins 1;
...
```{r IFig, echo=FALSE, results='hide', message=FALSE, ref.label='ICode', fig.cap="Fonctions d'information utilisées dans le nombre d'espèces (trait plein), l'indice de Shannon (pointillés longs) et l'indice de Simpson (pointillés). L'information apportée par l'observation d'espèces rares décroît du nombre d'espèces à l'indice de Simpson."}
```
...
```{r EntropieFig, echo=FALSE, results='hide', message=FALSE, ref.label='EntropieCode', fig.cap="Valeur de $p_{s}I\\left(p_s\\right)$ dans le nombre d'espèces (trait plein), l'indice de Shannon (pointillés longs) et l'indice de Simpson (pointillés). Les espèces rares contribuent peu, sauf pour le nombre d'espèces."}
```
...
```{r KullbackLeiblerFig, echo=FALSE, message=FALSE, results='hide', ref.label='KullbackLeiblerCode', fig.cap="Valeur de $q\\ln(\\frac{q}{p})$ en fonction de $p$ et $q$. La divergence de Kullback-Leibler est la somme de cette valeur pour toutes les espèces"}
```
...
Parmi les possibilités, $I(p_s) = \cos{(p_s \frac{\pi}{2})}$ est envisageable par exemple: le choix de la fonction d'information est virtuellement illimité, mais seules quelques unes seront interprétables clairement.
...
Par exemple, @Ricotta2003c proposent d'utiliser la fonction d'information $I(p_s)=-\ln(k_s)$ où $k_s$ est la dissimilarité totale de l'espèce $s$ avec les autres (par exemple, la somme des distances aux autres espèces dans un arbre phylogénétique, voir page~\@ref(sec:Dphylo)), normalisée pour que $\sum_s{k_s}=1$.
...
@Ricotta2006b ont défini plus tard une entropie augmentant avec l'originalité de chaque espèce, présentée au chapitre&nbsp;\@ref(sec:LeinsterCobbold)).
...
Ces résultats ont été complétés par d'autres et repris en écologie par @Keylock2005 et @Jost2006, Jost2007. 
Là encore:
...
En raison de leurs propriétés, notamment de décomposition (voir le chapitre&nbsp;\@ref(Decomposition)), @Jost2007 les appelle "vraie diversité". 
@Hoffmann2008 critiquent cette définition totalitaire^[@Jost2009 reconnaît qu'un autre terme aurait pu être choisi ("diversité neutre" ou "diversité mathématique" par exemple).]
...
@Leinster2012, après @Hill1973, @Patil1982, @Tothmeresz1995 et @Kindt2006, recommandent de tracer des profils de diversité, c'est-à-dire la valeur de la diversité $^{q}\!D$ en fonction de l'ordre $q$ (Figure&nbsp;\@ref(ProfilDivFig)) pour comparer plusieurs communautés.
...
@Lande2000 montrent que si la diversité de Simpson et la richesse de deux communautés n'ont pas le même ordre, alors les courbes d'accumulation du nombre d'espèces en fonction du nombre d'individus échantillonnés se croisent aussi.
  ...
  ```{r ProfilDivFig, echo=FALSE, results='hide', ref.label='ProfilDivCode', fig.cap="Profil de diversité calculé pour deux parcelles de Paracou (Parcelle 6: trait plein et Parcelle 18: trait pointillé). La correction du biais d'estimation est celle de Chao et Jost."}
```
...
Ils montrent que des communautés peuvent être séparables en selon un profil de diversité de $^{q}\!D$ sans l'être forcément selon un profil de diversité de Hurlbert (page&nbsp;\@ref(sec:Hurlbert)), et inversement.
...
@Zhang2014 montrent que toutes les mesures d'entropie usuelles sont des combinaisons linéaires de ${\zeta}_{u,v}$ (voir page~\@ref(eq:zeta)). 
...
@Chao2015 complètent cet estimateur par une estimation du biais résiduel, selon la même démarche que pour l'entropie de Shannon [@Chao2013] (voir page~\@ref(eq:Chao2013)).
...
Cet estimateur est le plus performant. Quand $q \to 1$, il tend vers l'estimateur de l'entropie de Shannon de @Chao2013, équation&nbsp;\@ref(eq:Chao2013) page~\@ref(eq:Chao2013). Quand $q=0$, l'estimateur se réduit à Chao1. Pour toutes les valeurs entières de $q$ supérieures ou égales à 2, il se réduit à l'estimateur (\@ref(eq:HqZhang)), identique à l'estimateur de Lande pour $q=2$, et est sans biais.

L'estimateur est obtenu à partir de celui du taux de couverture extrapolé (page~\@ref(sec:DefCouverture)) fourni par  @Chao2012b, annexe E, sans le détail de la démonstration présenté ici.
...
Le deuxième terme de la somme est estimé en posant $\hat{p}_s = \hat{\alpha}_0$ pour toutes les espèces non observées, ce qui constitue la même approximation que pour l'estimation de la relation de Good-Turing, équation~(\@ref(eq:GoodTuring2014)) page~\@ref(eq:GoodTuring2014).
Alors, $\sum_s{p_s}$ est estimé par $\hat{\alpha}_0 {s^{n}_{0}} = \frac{s^{n}_{1}}{n}(1 - \hat{\alpha}_1)$ et $(1-p_s)^m$ par $(1-\hat{\alpha}_0)^m$.
...
L'estimateur de $h_q$ est obtenu en remplaçant les termes $\zeta_{1,k}$ de la somme infinie $\sum_{k=n}^{\infty}{\binom{q-1}{k}(-1)^k \zeta_{1,k}}$ par la valeur de $1 -{\mathbb E}(C^{n+m})$
calculée dans l'équation~\eqref{eq:Cnm}.
La formule obtenue contient le développement en série entière de $[1 - (1-A)^{q-1}]$, ce qui permet d'éliminer la somme infinie et d'obtenir l'estimateur de l'équation~\eqref{eq:HqChaoJost} après simplifications [ @Chao2015, Appendix S1, Theorem S1.2d,].
...
L'estimateur naïf de la probabilité d'une espèe observée $s$ est $\hat{p}_s = \frac{n_s}{n}$.
...
Il est obtenu par @Chao2013 à partir de l'espérance conditionnelle de $\frac{N_s}{n}$:
...
```{r Estimation, fig.cap="Estimation de la diversité d'une communauté log-normale de 300 espèces. Un inventaire de 500 (en bas) ou 5000 (en haut) individus est répété 1000 fois et la diversité estimée à chaque fois par l'estimateur de Chao-Wang-Jost (à gauche) ou l'estimateur révélé (à droite). La courbe noire en gras représente la vraie diversité de la communauté. La courbe maigre est la moyenne de l'estimation, l'enveloppe grise limitée par les pointillés rouges contient 95\\% des valeurs simulées. A: 5000 individus, estimateur de Chao-Wang-Jost. B: 5000 individus, estimateur révélé. C:500 individus, estimateur de Chao-Wang-Jost. D: 500 individus, estimateur révélé.", echo=FALSE}
...
Le package *iNEXT* [@Hsieh2014] implémente ces calculs dans R.
...
Son défaut est qu'elle n'est pas forcément concave [@Beck2009], ce qui empêche sa décomposition (voir chapitre&nbsp;\@ref(chap:DedompHCDT), page~\@ref(chap:DedompHCDT)).
...
### L'entropie de Simpson généralisée

L'entropie de Simpson généralisée a été introduite dans une classe d'indices plus générale (voir page~\@ref(eq:zeta)) [@Zhang2010] et étudiée en détail par @Zhang2014.
...
La figure&nbsp;\@ref(comProfilzeta) montre les profils de diversité (en nombre effectifs d'espèces) des deux parcelles 6 et 18 de Paracou: 1&nbsp;ha inventorié, respectivement 681 et 483 arbres, pour une richesse estimée à 254 et 309 espèces par les estimateurs jackknife d'ordres 2 et 3:
```{r Paracou618S}
...
```{r comProfilzetaFig, echo=FALSE, results='hide', message=FALSE, out.width='.45\\maxwidth', ref.label='comProfilzetaCode', fig.cap="Comparaison des profils de diversité des parcelles 6 et 18 de Paracou avec leur intervalle de confiance à 95\\%. La parcelle 18 est tracée en rouge, lignes continues, et la 6 en vert, lignes pointillées. Diversité généralisée de Simpson (nombres effectif d'espèces): les profils sont distincts, ce qui permet de conclure que la parcelle 6 est plus diverse que la parcelle 18."}
```

```{r comProfilHCDTFig, echo=FALSE, results='hide', message=FALSE, out.width='.45\\maxwidth', ref.label='comProfilHCDTCode', fig.cap="Comparaison des profils de diversité des parcelles 6 et 18 de Paracou. Diversité HCDT: les profils se chevauchent jusqu'à $q \approx 0,7$."}
```

La diversité est donc estimée jusqu'à l'ordre 253.

Code R pour réaliser la figure&nbsp;\@ref(comProfilzeta):
...
En comparaison, la figure&nbsp;\@ref(comProfilHCDT) montre les profils de diversité HCDT des mêmes parcelles.
Ils se chevauchent pour les petits ordres de diversité, ce qui ne permet pas de conclure.

Code R pour réaliser la figure nbsp;\@ref(comProfilHCDT):
```{r comProfilHCDTCode, eval=FALSE, tidy=FALSE}
...
```{r zetaDiffFig, echo=FALSE, results='hide', ref.label='zetaDiffCode', fig.cap="Différence entre les entropies de Simpson généralisées des parcelles 18 et 6 de Paracou avec son intervalle de confiance à 95\\%. La ligne horizontale représente l'hypothèse nulle d'égalité entre les entropies, qui peut être rejetée."}
```
...
```{r Tuomisto2012, fig.cap="Courbes de Lorenz de plusieurs jeux de données. Les espèces sont classées par fréquence croissante, et la courbe représente la fréquence cumulée des espèces. Plus la courbe est loin de la bissectrice, plus le jeu de données est inéquitable.", echo=FALSE}
knitr::include_graphics('images/Tuomisto2012.png')
```

Un critère supplémentaire établi par @Smith1996, complété par @Gosselin2001 est la compatibilité avec la courbe de @Lorenz1905, construite en classant les espèces de la moins fréquente à la plus fréquente, et en traçant le cumul des fréquences en fonction du rang (Figure&nbsp;\@ref(Tuomisto2012), @Tuomisto2012): une communauté parfaitement équitable est représentée par la bissectrice.
...
La valeur minimale de $O$ est $\frac{1}{S}$.
...
$E_{1,0} = \frac{^{1}\!D}{^{0}\!D}$ avait déjà été proposé par @Sheldon1969.
...
```{r ProfilIneqFig, echo=FALSE, results='hide', ref.label='ProfilIneqCode', fig.cap="Profil d'inéquitabilité de BCI: L'inéquitabilité augmente quand on néglige de plus en plus les espèces rares. Pour q=2, la forêt de BCI est aussi inéquitable qu'une communauté de 6 espèces dans laquelle une seule serait représentée au-delà d'un nombre négligeable d'individus."}
```
...
L'inéquitabilité $1/{^{q}\!E}$ (Figure&nbsp;\@ref(ProfilIneqFig)) est un nombre effectif d'espèces: c'est la richesse d'une communauté du même niveau d'équitabilité que la communauté réelle dans laquelle tous les individus (sauf un nombre négligeable) appartiendraient à une seule espèce.

Code R pour la Figure&nbsp;\@ref(ProfilIneqFig):
...
```{r BCIEqFig, echo=FALSE, results='hide', ref.label='BCIEqCode', fig.cap="Profil d'équitabilité de BCI: représentation alternative de la Figure&nbsp;\\@ref(ProfilIneqFig). L'équitabilité est ici normalisée selon Jost."}
```
...
Les figures  montre que l'entropie et l'équitabilité sont corrélées positivement à la richesse dans des communautés log-normales de même variance.

```{r SJLogNormale, fig.cap="Relation entre l'entropie de Shannon (a), l'indice d'équitabilité de Pielou (b), l'entropie de Simpson (c), l'équitabilité de Hurlbert (d) et la richesse de communautés log-normales similaires (même écart-type des probabilités, seule la richesse varie).", fig.subcap= c("Entropie de Shannon en fonction de la richesse.", "Indice de Pielou en fonction de la richesse.", "Entropie de Simpson en fonction de la richesse", "Équitabilité de Hurlbert en fonction de la richesse"), out.width='.49\\linewidth', fig.asp=1, fig.ncol = 2, echo=FALSE}
...
```{r SJLogNormaleHCode, eval=FALSE}
# Communautés de 30 à 300 espèces
S <- 30:300
# Tirage de communautés log-normales de 10000 individus
rcl <- lapply(S, function(S) rCommunity(1, 10000, S=S))
...
```{r SJLogNormaleJCode, eval=FALSE}
# Equitabilité de Pielou
J <- H/log(S)
ggplot(data.frame(S, J), aes(S, J)) +
  geom_point()
...
```{r SJLogNormaleECode, eval=FALSE}
E <- sapply(rcl, Simpson)
ggplot(data.frame(S, E), aes(S, E)) +
  geom_point()
```

```{r SJLogNormaleJ2Code, eval=FALSE}
J2 <- E/(1-1/S)
ggplot(data.frame(S, J2), aes(S, J2)) +
  geom_point()
```
...
```{r NJLogNormale, fig.cap="Relation entre l'entropie de Shannon (a), l'indice d'équitabilité de Pielou (b), l'entropie de Simpson (c), l'équitabilité de Hurlbert (d) et la richesse d'échantillons de taille variable d'une même communauté log-normale.", fig.subcap= c("Entropie de Shannon en fonction de la richesse.", "Indice de Pielou en fonction de la richesse.", "Entropie de Simpson en fonction de la richesse", "Équitabilité de Hurlbert en fonction de la richesse"), out.width='.49\\linewidth', fig.asp=1, fig.ncol = 2, echo=FALSE}
...
Code R pour réaliser les figures:
```{r NJLogNormaleHCode, eval=FALSE}
# Communautés de taille 10 à 1000 individus; 300 sp.
rmc <- MetaCommunity(sapply(10:1000, function(n) rCommunity(1, n)))
# Richesse et Shannon
S <- apply(rmc$Psi, 2, Richness)
...
ggplot(data.frame(S, H), aes(S, H)) +
  ...
  labs(y="S (échelle logarithmique)")
```
```{r NJLogNormaleJCode, eval=FALSE}
# Equitabilité de Pielou
J <- H/log(S)
ggplot(data.frame(S, J), aes(S, J)) +
  geom_point()
```
```{r NJLogNormaleECode, eval=FALSE}
E <- apply(rmc$Psi, 2, Simpson)
ggplot(data.frame(S, E), aes(S, E)) +
  geom_point()
```
```{r NJLogNormaleJ2Code, eval=FALSE}
J2 <- E/(1-1/S)
ggplot(data.frame(S, J2), aes(S, J2)) +
  geom_point()
```
