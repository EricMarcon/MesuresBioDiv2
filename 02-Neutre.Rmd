...
```{r PrestonSansOFig, echo=FALSE, results='hide', ref.label='PrestonSansO2', fig.cap="Ajustement du modèle de Preston aux données de BCI."}
```
...
```{r KullbackLeiblerFig, echo=FALSE, message=FALSE, results='hide', ref.label='KullbackLeiblerCode', fig.cap="Valeur de $q\\ln(q/p)$ en fonction de $p$ et $q$. La divergence de Kullback-Leibler est la somme de cette valeur pour toutes les espèces", fig.asp=1}
```
...
calculée dans l'équation \@ref(eq:Cnm).
