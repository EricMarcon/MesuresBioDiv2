...
```{r BiaisJack1Code, eval=FALSE, size='tiny'}
...
```{r BiaisJack3Code, eval=FALSE, tidy=FALSE, size='tiny'}
...
```{r NonObsN, eval=FALSE, tidy=FALSE, size='tiny'}
...
Le code R nécessaire pour réaliser la figure est :
```{r PrestonSansO2, eval=FALSE, size='tiny'}
plot(mod.ll)
```
...
Le code R nécessaire pour réaliser la figure est:
```{r Lineweaver2, eval=FALSE, size='tiny', tidy = FALSE}
...
```{r ShannonCode, eval=FALSE, tidy=FALSE, size='tiny'}
...
```{r ShannonBiaisCode, eval=FALSE, size='tiny', tidy=FALSE}
Biais.p <- function (p) {
...
Code R:
```{r EntropieCode, eval=FALSE, size='tiny'}
H0 <- function(p) 1-p
...
Le code R nécessaire pour réaliser la figure est:
```{r KullbackLeiblerCode, eval=FALSE, size='tiny', message=FALSE, fig.asp=1}
p <- q <- seq(0.01, 1, .01)
...
Le code R nécessaire pour réaliser la figure est:
```{r lnqCode, eval=FALSE, size='tiny', tidy=FALSE}
...
```{r ProfilDivCode, eval=FALSE, size='tiny', tidy=FALSE}
q.seq <- seq(0, 2, .1)
...
Code R :
```{r ProfilQueueCode, eval=FALSE, size='tiny'}
...
```{r comProfilzetaCode, eval=FALSE, size='tiny', tidy=FALSE}
# Profil P6
...
```{r comProfilHCDTCode, eval=FALSE, size='tiny', tidy=FALSE}
...
Code R pour réaliser la figure:
```{r zetaDiffCode, eval=FALSE, size='tiny', tidy=FALSE}
...
```{r ProfilIneqCode, eval=FALSE, size='tiny'}
...
```{r BCIEqCode, eval=FALSE, size='tiny'}
...
```{r SJLogNormaleHCode, eval=FALSE, size='tiny'}
# Communautés de 30 à 300 espèces
S <- 30:300
# Tirage de communautés log-normales de 10000 individus
rcl <- lapply(S, function(S) rCommunity(1, 10000, S=S))
...
```{r SJLogNormaleJCode, eval=FALSE, size='tiny'}
# Equitabilité de Pielou
J <- H/log(S)
ggplot(data.frame(S, J), aes(S, J)) +
  geom_point()
...
```{r SJLogNormaleECode, eval=FALSE, size='tiny'}
E <- sapply(rcl, Simpson)
ggplot(data.frame(S, E), aes(S, E)) +
  geom_point()
```

```{r SJLogNormaleJ2Code, eval=FALSE, size='tiny'}
J2 <- E/(1-1/S)
ggplot(data.frame(S, J2), aes(S, J2)) +
  geom_point()
```
...
Code R pour réaliser les figures:
```{r NJLogNormaleHCode, eval=FALSE, size='tiny'}
# Communautés de taille 10 à 1000 individus; 300 sp.
rmc <- MetaCommunity(sapply(10:1000, function(n) rCommunity(1, n)))
# Richesse et Shannon
S <- apply(rmc$Psi, 2, Richness)
...
ggplot(data.frame(S, H), aes(S, H)) +
  ...
  labs(y="S (échelle logarithmique)")
```
```{r NJLogNormaleJCode, eval=FALSE, size='tiny'}
# Equitabilité de Pielou
J <- H/log(S)
ggplot(data.frame(S, J), aes(S, J)) +
  geom_point()
```
```{r NJLogNormaleECode, eval=FALSE, size='tiny'}
E <- apply(rmc$Psi, 2, Simpson)
ggplot(data.frame(S, E), aes(S, E)) +
  geom_point()
```
```{r NJLogNormaleJ2Code, eval=FALSE, size='tiny'}
J2 <- E/(1-1/S)
ggplot(data.frame(S, J2), aes(S, J2)) +
  geom_point()
```
