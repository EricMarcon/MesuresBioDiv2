...
Une relation entre le nombre de doubletons et les données disponibles est fourni; sa résolution numérique (le code R nécessaire est disponible avec l'article) permet d'estimer $s^{n}_{2}$ et de l'injecter dans l'estimateur Chao2.
...
```{r BiaisJack1Code, eval=FALSE, size='footnotesize'}
...
```{r BiaisJack3Code, eval=FALSE, tidy=FALSE, size='footnotesize'}
...
```{r NonObsN, eval=FALSE, tidy=FALSE, size='footnotesize'}
...
Le code R nécessaire pour réaliser la figure est:
```{r PrestonSansO2, eval=FALSE, size='footnotesize'}
plot(mod.ll)
```
...
La courbe d'accumulation de BCI a été calculée en figure \@ref(fig:SACFig).
...
Le code R nécessaire pour réaliser la figure est:
```{r Lineweaver2, eval=FALSE, size='footnotesize', tidy = FALSE}
...
L'estimation est cette fois supérieure à celle du jackknife (`r round(SPECIES::jackknife(entropart::AbdFreqCount(Ns))$Nhat, 0)` espèces).
...
```{r ShannonCode, eval=FALSE, tidy=FALSE, size='footnotesize'}
...
```{r ShannonBiaisCode, eval=FALSE, size='footnotesize', tidy=FALSE}
Biais.p <- function (p) {
...
Le code R nécessaire pour réaliser la figure est:
```{r ICode, eval=FALSE, size='footnotesize'}
I0 <- function(p) (1-p)/p
I1 <- function(p) -log(p)
I2 <- function(p) 1-p
...
Code R:
```{r EntropieCode, eval=FALSE, size='footnotesize'}
H0 <- function(p) 1-p
...
Le code R nécessaire pour réaliser la figure est:
```{r KullbackLeiblerCode, eval=FALSE, size='footnotesize', message=FALSE, fig.asp=1}
p <- q <- seq(0.01, 1, .01)
...
Le code R nécessaire pour réaliser la figure est:
```{r lnqCode, eval=FALSE, size='footnotesize', tidy=FALSE}
...
```{r ProfilDivCode, eval=FALSE, size='footnotesize', tidy=FALSE}
q.seq <- seq(0, 2, .1)
...
Code R:
```{r ProfilQueueCode, eval=FALSE, size='footnotesize'}
...
@Chao2015 complètent cet estimateur par une estimation du biais résiduel, selon la même démarche que pour l'entropie de Shannon [@Chao2013] \@ref(eq:Chao2013).
...
```{r comProfilzetaCode, eval=FALSE, size='footnotesize', tidy=FALSE}
# Profil P6
...
```{r comProfilHCDTCode, eval=FALSE, size='footnotesize', tidy=FALSE}
...
Code R pour réaliser la figure:
```{r zetaDiffCode, eval=FALSE, size='footnotesize', tidy=FALSE}
...
```{r ProfilIneqCode, eval=FALSE, size='footnotesize'}
...
```{r BCIEqFig, echo=FALSE, results='hide', ref.label='BCIEqCode', fig.cap="Profil d'équitabilité de BCI. L'équitabilité est ici normalisée selon Jost."}
```
...
```{r BCIEqCode, eval=FALSE, size='footnotesize'}
...
```{r SJLogNormaleHCode, eval=FALSE, size='footnotesize'}
# Communautés de 30 à 300 espèces
S <- 30:300
# Tirage de communautés log-normales de 10000 individus
rcl <- lapply(S, function(S) rCommunity(1, 10000, S=S))
...
```{r SJLogNormaleJCode, eval=FALSE, size='footnotesize'}
# Equitabilité de Pielou
J <- H/log(S)
ggplot(data.frame(S, J), aes(S, J)) +
  geom_point()
...
```{r SJLogNormaleECode, eval=FALSE, size='footnotesize'}
E <- sapply(rcl, Simpson)
ggplot(data.frame(S, E), aes(S, E)) +
  geom_point()
```

```{r SJLogNormaleJ2Code, eval=FALSE, size='footnotesize'}
J2 <- E/(1-1/S)
ggplot(data.frame(S, J2), aes(S, J2)) +
  geom_point()
```
...
Code R pour réaliser les figures:
```{r NJLogNormaleHCode, eval=FALSE, size='footnotesize'}
# Communautés de taille 10 à 1000 individus; 300 sp.
rmc <- MetaCommunity(sapply(10:1000, function(n) rCommunity(1, n)))
# Richesse et Shannon
S <- apply(rmc$Psi, 2, Richness)
...
ggplot(data.frame(S, H), aes(S, H)) +
  ...
  labs(y="S (échelle logarithmique)")
```
```{r NJLogNormaleJCode, eval=FALSE, size='footnotesize'}
# Equitabilité de Pielou
J <- H/log(S)
ggplot(data.frame(S, J), aes(S, J)) +
  geom_point()
```
```{r NJLogNormaleECode, eval=FALSE, size='footnotesize'}
E <- apply(rmc$Psi, 2, Simpson)
ggplot(data.frame(S, E), aes(S, E)) +
  geom_point()
```
```{r NJLogNormaleJ2Code, eval=FALSE, size='footnotesize'}
J2 <- E/(1-1/S)
ggplot(data.frame(S, J2), aes(S, J2)) +
  geom_point()
```
