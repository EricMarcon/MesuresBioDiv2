...
```{r Anderson2011a, fig.cap="Diversité $\\beta$ liée au remplacement des espèces le long d'un gradient. $\\Delta{y}$ est la mesure de la dissimilarité entre deux communautés. $\\mathbf{x}$ est un gradient environnemental, spatial ou temporel. $\\Delta{x}$ est l'écart entre deux points du gradient.", echo=FALSE}
...
Cette question est traitée en détail en section~\@ref(sec:DiversiteJointe).
...
```{r JaccardCode, eval=FALSE, size='footnotesize'}
...
Code R pour la figure:
```{r ChaoJaccardCode, eval=FALSE, size='footnotesize', tidy=FALSE}
...
Code R pour la figure:
```{r JaccardPCOCode, eval=FALSE, size='footnotesize'}
...
Code R pour la figure:
```{r BCIPCACode, eval=FALSE, size='footnotesize'}
...
Code R:
```{r BCIPCA2Code, eval=FALSE, size='footnotesize'}
...
Code R:
```{r BCICordeCode, eval=FALSE, size='footnotesize'}
library("adespatial")
...
```{r BetaSampleCode1, eval=FALSE, size='footnotesize', tidy=FALSE}
library("gplots")
...
Code pour réaliser la figure:
```{r DivPartCode, eval=FALSE, size='footnotesize', tidy=FALSE}
q <- 1
...
Code R:
```{r DivestCode, eval=FALSE, size='footnotesize'}
