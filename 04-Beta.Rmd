...
```{r Notations, echo=FALSE, warning=FALSE}
...
knitr::kable(Notations, caption="Notations des effectifs, tableau espèces-communautés.", longtable = FALSE, booktabs = TRUE, escape = FALSE) %>%
...
Ces deux définitions sont présentées sur les figures&nbsp;\@ref(Anderson2011a) et \@ref(Anderson2011b)[@Anderson2011].

```{r Anderson2011a, fig.cap="Diversité $\\beta$ liée au remplacement des espèces le long d'un gradient. $\\Delta{y}$ est la mesure de la dissimilarité entre deux communautés, voir page~\\@ref(chap:BetaPaires). $\\mathbf{x}$ est un gradient environnemental, spatial ou temporel. $\\Delta{x}$ est l'écart entre deux points du gradient.", echo=FALSE}
knitr::include_graphics('images/Anderson2011a.png')
```

```{r Anderson2011b, fig.cap="Diversité $\\beta$ liée à la variation de la composition spécifique. \\beta_W$ est le rapport entre la diversité $\\gamma$ et la diversité $\\alpha$, $\\beta_{Add}$ leur différence. $d_{ij}$ est la dissimilarité entre les communautés $i$ et $j$. $\\hat{\\sigma}^2$ est la variance de cette dissimilarité, $\\bar{d}$ sa moyenne. Si $d$ est une distance euclidienne, $\\bar{d}_{cen}$ est la distance moyenne des communautés à leur centre de gravité.", echo=FALSE}
...
De nombreuses mesures de dissimilarité existent, elles sont détaillées dans le chapitre&nbsp;\@ref(chap:BetaPaires), à partir de la page~\@ref(chap:BetaPaires).
...
Une variation sur le modèle T2 consiste à le représenter sous la forme d'un modèle de décroissance en fonction de la distance [@Nekola1999], éventuellement pour plusieurs groupes de communautés (Figure&nbsp;\@ref(Anderson2011a), T4 et T5). 
L'indice de Simpson spatialement explicite (page~\@ref(sec:SimpsonSpatial)) entre dans ce cadre bien qu'il soit construit différemment.
...
Dans son article fondateur, @Whittaker1960 définit la diversité $\beta$ comme le niveau de changement dans la composition des communautés, la nomme "diversité secondaire " et lui attribue la lettre $\beta$ en référence à la diversité "primaire " mesurable par l'indice $alpha$ de Fisher [@Fisher1943] (voir le chapitre \@ref(chap:Fisher), page \@ref(chap:Fisher)).
...
@Whittaker1960, Whittaker1972 est l'auteur de ce concept. 
...
### Définitions de la diversité $\alpha$
\label{sec:defalpha}
...
\begin{equation}
  ^{q}\!D_{\alpha}={\left(\sum_s{\sum_i{w_ip^q_{s|i}}}\right)}^{1/{\left(1-q\right)}}.
\end{equation}
...
Cette question est traitée en détail page~\@ref(sec:DiversiteJointe).
...
Deux autres indices sont mentionnés ici, celui de @Sokal1963 et celui d'@Ochiai1957 :
...
```{r JaccardFig, echo=FALSE, results='hide', out.width='\\textwidth', ref.label='JaccardCode', fig.cap="Distribution des indices de Jaccard calculés pour les 50 carrés de BCI deux à deux."}
```
...
```{r ChaoJaccardFig, echo=FALSE, results='hide', out.width='\\textwidth', ref.label='ChaoJaccardCode', fig.cap="Distribution des indices de Jaccard corrigés du biais d'etimation pour les 50 carrés de BCI deux à deux."}
```
...
```{r JaccardPCOFig, echo=FALSE, results='hide', out.width='\\textwidth', ref.label='JaccardPCOCode', fig.cap="Représentation des 50 carrés de BCI dans une analyse en coordonnées principale fondée sur les distances de Jaccard."}
```
...
La dissimilarité de Ružička [@Ruzicka1958] généralise celle de Jaccard :
...
```{r Podani2011, fig.cap="Triangle du simplex Similarité-Substitution-Emboîtement. (a) Similarité, emboîtement et substitution somment à 1 donc les trois composantes peuvent être représentées dans un triangle et combinées deux à deux. La dissimilarité est par exemple représentée par la proximité au côté gauche du triangle, somme de l'emboîtement et de la substitution (ses sommets adjacents) et complément à 1 de la dissimilarité (le sommet opposé). (b) Les formules de calcul dont données pour la similarité de Jaccard. Avec ces notations classiques, $a$ est le nombre d'espèces communes $s_{1,1}$, $b$ et $c$ sont les espèces propres à chaque communauté, $s_{0,1}$ et $s_{1,0}$, et $n$ est le nombre total d'espèces $a+b+c$.", echo=FALSE}
...
@Podani2011 représentent cette égalité dans un triangle dont les sommets correspondent aux trois termes (Figure \@ref(Podani2011)).
...
$\pi_{s(n)}$ désigne la contribution de l'espèce $s$ à l'indice de Hurlbert, déjà vue dans le cadre de l'originalité taxonomique (page~\@ref(sec:OrigTax)).
La contribution des espèces rares augmente avec le paramètre $n$ (Figure&nbsp;\@ref(HurlbertCFig), page~\@ref(HurlbertCFig)).
...
Une matrice de similarité $\mathbf{Z}$ est définie entre les paires d'espèces: $z_{s,t}$ est la similarité entre l'espèce $s$ et l'espèce $t$, comprise entre 0 et 1 de la même façon que pour le calcul de la diversité $\xq{D}{q}{\mathbf{Z}}{}{}$ (page~\@ref(sec:LeinsterCobbold)). 
...
La dissimilarité entre communautés de Rao (page~\@ref(sec:RaoDisc)) peut être normalisée (en la divisant par sa valeur maximale pour qu'elle soit comprise entre 0 et 1) puis transformée en similarité (en prenant son complément à 1) pour obtenir
...
```{r BCIPCAFig, echo=FALSE, results='hide', out.width='\\textwidth', ref.label='BCIPCACode', fig.cap="Représentation des deux premiers axes factoriels de l'ACP des 50 carrés de BCI. Les données sont les probabilités des espèce, ni centrées ni réduites pour l'ACP. Les carrés sont représentés avec les espèces. Les carrés 35 et 40 ont une diversité faible et sont associés aux espèces *Gustavia superba* et *Alseis blackiana*."}
```
...
Cette "définition" est valide puisque cette distance est la composante inter de la décomposition de l'entropie $\beta$ de Simpson (page~\@ref(eq:SimpsonNei)).
...
```{r BCIPCA2Fig, echo=FALSE, results='hide', out.width='\\textwidth', ref.label='BCIPCA2Code', fig.cap="ACP centrée des 50 carrés de BCI pour visualiser la diversité $\beta$."}
```


L'ACP centrée (mais non réduite, figure&nbsp;\@ref(BCIPCA2Fig)) permet de mieux représenter la diversité $\beta$ : la distance de chaque carré au centre du repère est sa contribution.
...
```{r Corde, fig.cap="Distance de corde entre deux communautés représentées dans l'espace engendré par deux espèces. La distance $d_{1,2}$ entre les deux communautés est la longueur de la corde qui relie leurs vecteurs de composition normalisés sur le cercle de rayon 1. C'est aussi deux fois le sinus de l'angle $\\frac{\\alpha_{1,2}}{2}$ formé par les deux vecteurs de composition des communautés.", echo=FALSE}
...
```{r BCICordeFig, echo=FALSE, message=FALSE, results='hide', out.width='\\textwidth', ref.label='BCICordeCode', fig.cap="PCoA appliquée à la distance de corde des 50 carrés de BCI pour visualiser la diversité $\beta$."}
```

La diversité $\beta$ de BCI (Figure&nbsp;\@ref(fig:BCIPCA2Fig)) est mieux représentée par une analyse en coordonnées principales appliquée à la distance de corde entre les communautés (Figure&nbsp;\@ref(BCICordeFig)).
...
# Décomposition de l'entropie HCDT
...
Un forum a été consacré à la question dans la revue *Ecology*, conclu par @Chao2012a, et une revue a été faite par @Tuomisto2010a, Tuomisto2010b.
...
```{r DecShannon, echo=FALSE, warning=FALSE}
...
knitr::kable(DecShannon, caption="Définition explicite de la diversité de Shannon en tant que divergence entre une distribution observée et une distribution attendue. Les indices $H_\\alpha$ et $H_\\gamma$ sont les compléments au logarithme du nombre d'espèces de cette divergence.", longtable = FALSE, booktabs = TRUE, escape = FALSE) %>%
  kableExtra::kable_styling(bootstrap_options = "striped", full_width = TRUE)
```
...
\toprule
Indice &  & Distribution attendue & Formule \\
...
La décomposition explicite (Tableau&nbsp;\@ref(table:Shannon)) est due à @Marcon2012a mais avait été établie plus ou moins clairement plusieurs fois dans la littérature, notamment par @Rao1985 : la diversité $\beta$ est la divergence de Kullback-Leibler entre la distribution des espèces dans chaque parcelle et la distribution moyenne, appelée divergence de Jensen-Shannon [@Lin1991]. 
...
Le meilleur estimateur peut être évalué à partir de la différence entre les entropies $\gamma$ et $\alpha$, corrigées par le meilleur estimateur disponible (celui de Chao, Wang et Jost, voir page~\@ref(sec:BiaisShannon)):
...
* La population totale étant partitionnée, chaque partition recevant un poids $w_i$, la composante intra-groupe de la mesure $H_{\alpha}$ est égale à la somme pondérée des mesures dans chaque-groupe $H_{\alpha}=\sum_i{w_i_{i}\!H_{\alpha}}$;
...
$^{q}\!H_{\gamma}$ peut s'écrire sous la forme $^{q}\!H_{\gamma} = -\sum_s{p^q_s}\ln_q{p_s}$: la formalisation de la décomposition est une généralisation de la décomposition de l'entropie de Shannon, résumée dans le Tableau&nbsp;\@ref(Decomposition).
...
Sa fonction d'information a été tracée Figure&nbsp;\@ref(IFig), page~\@ref(IFig).
...
La forme de l'entropie $\beta$ évoque la divergence de puissance [@Cressie1984] qui est une statistique qui mesure la divergence entre deux distributions (déjà mentionnée page~\@ref(CressieRead)), dont les propriétés asymptotiques sont connues. 
...
La communauté dont le poids $w_{min}$ est le plus faible est représentée une seule fois, les autres $\frac{w_i}{w_{min}}$ fois.
L'indépendance entre les diversités $\alpha$ et $\beta$ est donc bien vérifiée.
Le diversité $\beta$ maximale théorique est $\frac{1}{w_{min}}$ (et non $I$).
...
C'est précisément la définition de $^{0}\!D_{\beta}$ (équation \@ref(eq:D0beta)).
...
Cette décomposition est remise en cause par @Jost2007, Jost2008, qui a généré le débat [@Heller2009, Ryman2009, Jost2009a, Whitlock2011]. 
$E_{\gamma}$ étant inférieure ou égale à 1, $E_{\beta}$ n'est pas indépendante de $E_{\alpha}$: seule la décomposition multiplicative permet l'indépendance, et Jost propose d'utiliser une transformation de $^{2}\!D_{\beta}$ comme mesure de différenciation (voir page~\@ref(sec:Chevauchement)). @Gregorius2014 montre que ce problème n'est pas limité à l'entropie de Simpson mais s'applique à toutes les entropies HCDT d'ordre supérieur à 1.
...
```{r Decomposition, echo=FALSE, warning=FALSE}
...
knitr::kable(Decomposition, caption="Entropie, diversité et décomposition. L'entropie $\\alpha$ de chaque communauté se calcule comme l'entropie $\\gamma$", longtable = FALSE, booktabs = TRUE, escape = FALSE) %>%
  kableExtra::kable_styling(bootstrap_options = "striped", full_width = TRUE)
```
...
## Décomposition de l'indice de Rao
\label{sec:RaoDisc}

Les notations sont celles de la présentation de l'indice de Rao, page~\@ref(sec:Rao).
...
@Hardy2008 montrent que les deux pondérations sont valides mais l'absence de cadre général assurant que la diversité $\gamma$ de Rao est supérieure à la diversité $\alpha$ [@DeBello2010] motive @Guiasu2011 à proposer une alternative à l'indice de Rao, l'indice de Gini-Simpson quadratique pondéré (page~\@ref(eq:Guiasu)), dont la concavité est démontrée (ce qui implique que $\gamma\ge \alpha$). 
...
Ce résultat a été obtenu pour les arbres ultramétriques dans le cadre plus général de la décomposition de l'entropie phylogénétique (voir chapitre&nbsp;\@ref(chap:DedompHCDT) page~\@ref(chap:DedompHCDT)).
...
Pour une taille d'échantillon fixée (par exemple un hectare de forêt), la diversité $\beta$ ne dépend pas du nombre de communautés: l'entropie $\beta$ est l'espérance (estimée par la moyenne, équation&nbsp;\@ref(eq:ContributionBeta), page~\@ref(eq:ContributionBeta)) de la divergence de Jensen-Shannon entre la distribution des espèces d'une communauté et celle de la méta-communauté, fixe. 
...
```{r BetaSampleFig1, echo=FALSE, results='hide', message=FALSE, ref.label='BetaSampleCode1', fig.cap="Estimation de la diversité $\beta$ entre carrés de BCI. La diversité est calculée à partir du nombre de carrés en abscisse, de 2 à 30, tirés aléatoirement. La valeur en ordonnée est la diversité d'ordre 2 moyenne sur 100 tirages, les barres représentent l'écart-type. Aux erreurs d'estimation près, la valeur de la diversité $\beta$ ne dépend pas du nombre de carrés."}
```

La Figure&nbsp;\@ref(BetaSampleFig) présente les valeurs estimées de diversité neutre $\beta$ entre carrés de BCI tirés aléatoirement, en fonction du nombre de carrés.

Code pour réaliser la figure:
```{r BetaSampleCode1, eval=FALSE, tidy=FALSE}
library("gplots")
...
La décomposition de l'entropie phylogénétique au sens d'@Allen2009, c'est-à-dire dans la cas particulier $q=1$, a été établie par @Mouchet2011.
...
\begin{equation}
  \xq{H}{q}{\mathbf{Z}}{\alpha}{}
  = \sum_i{w_i\xq[\,]{H}{q}{\mathbf{Z}}{\alpha}{i}}
...
\begin{equation}
  \xq{D}{q}{\mathbf{Z}}{\beta}{}
=e^{\frac{\xq{H}{q}{\mathbf{Z}}{}{}}{1+\left(1-q\right)\xq[\,]{H}{q}{\mathbf{Z}}{\alpha}{}}}_q.
\end{equation}

La décomposition de la diversité peut être faite directement, sans passer par l'entropie.

L'inverse de $\xq{D}{q}{\mathbf{Z}}{\gamma}{}$ est la moyenne généralisée d'ordre $q-1$ de ${\left(\mathbf{Zp}\right)}_s$:

\begin{equation}
  \frac{1}{\xq{D}{q}{\mathbf{Z}}{\gamma}{}}
...
\begin{equation}
  \xq{D}{q}{\mathbf{Z}}{\beta}{}
={\left[\sum_i{w_i}{\left(\frac{1/\xq[\,]{D}{q}{\mathbf{Z}}{\alpha}{i}}{1/\xq[\,]{D}{q}{\mathbf{Z}}{\gamma}{}}\right)}^{q-1}\right]}^{\frac{1}{q-1}}.
\label{eq:Dbeta}
\end{equation}

De la même façon que $1/\xq[\,]{D}{q}{\mathbf{Z}}{\gamma}{}$ est la banalité moyenne des espèces, $\xq{D}{q}{\mathbf{Z}}{\beta}{}$ est la banalité moyenne normalisée des communautés, où la banalité normalisée de la communauté $i$ est définie comme $\frac{\left(1/\xq[\,]{D}{q}{\mathbf{Z}}{\alpha}{i}\right)}{\left(1/\xq[\,]{D}{q}{\mathbf{Z}}{\gamma}{}\right)}$, c'est-à-dire la banalité moyenne de ses espèces divisée par celle de la méta-communauté.
...
Ce chapitre ne traite pas de la diversité de différentiation: les tests correspondants ont été présentés page~\@ref(sec:TestBetaDiff).
...
```{r DivestFig, echo=FALSE, results='hide', ref.label='DivestCode', fig.cap="Estimation de la diversité de la méta-communauté `Paracou618`."}
```

Calcul sous R: la fonction `DivEst` produit un graphique avec l'intervalle de confiance (Figure&nbsp;\@ref(DivestFig)).
...
```{r Couteron, echo=FALSE, warning=FALSE}
Couteron <- data.frame(
...
knitr::kable(Couteron, caption="Tableau d'occurrence des espèces.", longtable = FALSE, booktabs = TRUE, escape = FALSE) %>%
  kableExtra::kable_styling(bootstrap_options = "striped", full_width = TRUE)
```
...
Si la dissimilarité n'est pas euclidienne, la concavité de $ADW_{\rho}$ n'est pas garantie, ce qui motive Kosman à proposer une autre mesure [@Kosman1996, Kosman2007], ${KW}_{\rho}$, qui est la dissimilarité moyenne des $n$ paires d'individus les plus dissimilaires (au lieu de toutes les $\frac{n(n-1)}{2}$ paires).
