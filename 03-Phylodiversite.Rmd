...
```{r HurlbertCCode, eval=FALSE, size='tiny'}
...
Code R:
```{r ScheinerPsfsCode, eval=FALSE, size='tiny', tidy=FALSE}
# Probabilités
...
```
  ...
  Code R:
```{r ProfilDqAFCode, eval=FALSE, size='tiny', tidy=FALSE}
...
```{r PCAFCode, eval=FALSE, size='tiny'}
scatter(pcaf)
```
...
```{r HclustCode, eval=FALSE, size='tiny'}
plot(hf, h=-1)
```
...
```{r PhylogRaoCode, eval=FALSE, size='tiny'}
...
```{r originalityCode, eval=FALSE, size='tiny'}
...
```{r MoyenneHardyCode, eval=FALSE, size='tiny', tidy=FALSE}
X <- runif(100)
...
Le code R nécessaire pour réaliser la figure est:
```{r LineaireZpsqCode, eval=FALSE, size='tiny'}
r <- 0.5
seqZp <- seq(0, 1, 0.001)
...
Le code R nécessaire pour réaliser la figure est:
```{r EchelleCode, eval=FALSE, size='tiny', tidy=FALSE}
...
```{r DqZLambdaCode, eval=FALSE, size='tiny'}
...
Le code R nécessaire pour réaliser la figure est:
```{r ComparaisonDqCode, eval=FALSE, size='tiny'}
q <- c(seq(0, 3, 0.1))
