...
```{r ArbreA1, fig.cap="Arbre phylogénétique ou fonctionnel hypothétique. 5 espèces sont présentes ($S=5$), leurs probabilités notées $p_1$ à $p_5$. Les noms des branches sont affichés.", echo=FALSE}
...
La transformation d'une matrice (non ultramétrique) en dendrogramme déforme la topologie des espèces [@Pavoine2005a; @Podani2007]: une mesure de diversité qui utilise directement la matrice est préférable, c'est un intérêt de la  diversité de Leinster et Cobbold (voir section \@ref(sec:LeinsterCobbold)).
...
```{r ArbreA4, fig.cap="Arbre phylogénétique ou fonctionnel hypothétique. L'arbre comprend 5 espèces dont les probabilités sont notées $p_s$, 3 périodes de durées $T_k$ délimitées par les noeuds. Les branches sont notées $b$ et indicées par la période à laquelle elles se terminent et un numéro d'ordre.", echo=FALSE}
knitr::include_graphics('images/ArbreA.png')
```
...
```{r HurlbertCCode, eval=FALSE, size='footnotesize'}
...
La divergence de l'espèce 1 de la figure \@ref(fig:Arbre) est $T1+T2$, la longueur de la branche terminale, plus $\frac{T3}{2}$, parce que la branche est partagée par les espèces 1 et 2. 
...
Code R:
```{r ScheinerPsfsCode, eval=FALSE, size='footnotesize', tidy=FALSE}
# Probabilités
...
```
  ...
  Code R:
```{r ProfilDqAFCode, eval=FALSE, size='footnotesize', tidy=FALSE}
...
```{r PCAFCode, eval=FALSE, size='footnotesize'}
scatter(pcaf)
```
...
```{r HclustCode, eval=FALSE, size='footnotesize'}
plot(hf, h=-1)
```
...
```{r PhylogRaoCode, eval=FALSE, size='footnotesize'}
...
```{r originalityCode, eval=FALSE, size='footnotesize'}
...
```{r ArbreA5, fig.cap="Arbre phylogénétique ou fonctionnel hypothétique. 5 espèces sont présentes ($S=5$), leurs probabilités notées $p_1$ à $p_5$. Les noms des branches sont affichés.", echo=FALSE}
knitr::include_graphics('images/ArbreA.png')
```
...
```{r MoyenneHardyCode, eval=FALSE, size='footnotesize', tidy=FALSE}
X <- runif(100)
...
Le code R nécessaire pour réaliser la figure est:
```{r LineaireZpsqCode, eval=FALSE, size='footnotesize'}
r <- 0.5
seqZp <- seq(0, 1, 0.001)
...
Le code R nécessaire pour réaliser la figure est:
```{r EchelleCode, eval=FALSE, size='footnotesize', tidy=FALSE}
...
```{r ArbreA6, fig.cap="Arbre phylogénétique ou fonctionnel hypothétique. 5 espèces sont présentes ($S=5$), leurs probabilités notées $p_1$ à $p_5$. Les noms des branches sont affichés.", echo=FALSE}
knitr::include_graphics('images/ArbreA.png')
```
...
```{r SimMatrice, echo=FALSE, warning=FALSE}
Matrice <- data.frame(
...
knitr::kable(Matrice, caption="Matrice de similarité correspondant à l'arbre phylogénétique en exemple. Les noms des lignes et colonnes sont les couples (espèce actuelle; branche ancestrale).", longtable = FALSE, booktabs = TRUE, escape = FALSE) %>%
...
```{r DqZLambdaCode, eval=FALSE, size='footnotesize'}
...
Le code R nécessaire pour réaliser la figure est:
```{r ComparaisonDqCode, eval=FALSE, size='footnotesize'}
q <- c(seq(0, 3, 0.1))
