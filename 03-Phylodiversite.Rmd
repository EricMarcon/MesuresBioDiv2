...
L'entropie phylogénétique (voir Section&nbsp;\@ref(chap:Phyloentropie)) utilise un arbre phylogénétique pour mesurer la diversité.
...
Dans l'arbre de la figure&nbsp;\@ref(fig:ArbreA4), les valeurs de TD non normalisées sont $\frac{1}{2}$ pour les deux premières espèces et $\frac{1}{3}$ pour les trois autres: la racine de l'arbre est comptabilisée dans le nombre de noeuds.
...
La divergence de l'espèce 1 de la figure&nbsp;\@ref(fig:Arbre), page~\@ref(Arbre), est $T1+T2$, la longueur de la branche terminale, plus $\frac{T3}{2}$, parce que la branche est partagée par les espèces 1 et 2. 
...
La matrice de similarité correspondant à l'arbre phylogénétique de la figure&nbsp;\@ref(fig:figArbreA6) est la suivante.
...
Pour un arbre non ultramétrique, il est possible de définir une matrice $\mathbf{Z}$ similaire à celle de la figure&nbsp;\@ref(fig:MatricePhylo), mais ses valeurs non nulles ne sont pas égales à 1.
...
$\xq{D}{q}{\mathbf{Z}}{}{}$ est moins sensible au paramètre $q$ que $^{q}\!D(T)$: la figure&nbsp;\@ref(fig:ComparaisonDqFig) compare les deux mesures.
