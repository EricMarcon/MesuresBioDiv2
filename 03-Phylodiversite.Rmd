...
```{r PhylogRaoFig, echo=FALSE, results='hide', ref.label='PhylogRaoCode', fig.cap="Présentation de l'arbre phylogénétique avec la contribution de chacune des variables."}
```
...
```{r LineaireZpsqFig, fig.cap = "Approximation de ${(\\mathbf{Zp})}_{s}^{q-1}$ par son développement limité au premier ordre pour (a) $q=1.5$ ($r=q-1$) et (b) $q=0.5$", out.width='100%', echo = FALSE, ref.label='LineaireZpsqCode', fig.asp=.4}
```
...
```{r EchelleFig, echo=FALSE, results='hide', ref.label='EchelleCode', fig.cap="Transformation des distances (en abscisse) en similarités (en ordonnées) selon la valeur de $u$. Trait plein: $u=1$; pointillés longs: $u=2$; pointillés: $u=10$. Plus $u$ est grand, plus les similarités sont tassées vers 0."}
```
...
```{r MatricePhylo, echo=FALSE, warning=FALSE, size="scriptsize"}
Matrice <- data.frame(
...
knitr::kable(Matrice, caption="Matrice de similarité correspondant à l'arbre phylogénétique en exemple. Les noms des lignes et colonnes sont les couples (espèce actuelle; branche ancestrale).", longtable = FALSE, booktabs = TRUE, escape = FALSE) %>%
  kableExtra::kable_styling(font_size = 8, bootstrap_options = "striped", full_width = TRUE)
```
