...
Les premières propositions de ce type d'indices sont dues à @Rao1982 (voir page~\@ref(sec:Rao)) puis, avec nettement moins de succès, @Vane-Wright1991 et @Warwick1995. 
...
De nombreuses mesures de diversité ont été créées et plusieurs revues permettent d'en faire le tour. [@Ricotta2007; @Vellend2010; @Pavoine2011]
...
```{r ArbreA1, fig.cap="Arbre phylogénétique ou fonctionnel hypothétique. Rappel de la figure \\@ref(Arbre)(a). 5 espèces sont présentes ($S=5$), leurs probabilités notées $p_1$ à $p_5$. Les noms des branches sont affichés.", echo=FALSE}
...
L'entropie phylogénétique (voir page~\@ref(chap:Phyloentropie)) utilise un arbre phylogénétique pour mesurer la diversité.
...
@Podani2006 suggèrent d'utiliser ensuite une classification hiérarchique par UPGMA [@Sokal1958] qu'ils montrent être la plus robuste (pour le calcul de FD, voir page~\@ref(sec:PDFD)) à l'ajout ou au retrait d'un trait ou d'une espèce.
...
La transformation d'une matrice (non ultramétrique) en dendrogramme déforme la topologie des espèces [@Pavoine2005a; @Podani2007] : une mesure de diversité qui utilise directement la matrice est préférable, c'est un intérêt de la  diversité de Leinster et Cobbold (voir page~\@ref(sec:LeinsterCobbold)).
...
```{r Pavoine2011, fig.cap="Régularité contre irrégularité. Les arbres de A à D sont de plus en plus irréguliers. L'arbre A, parfaitement régulier, est le cadre des mesures classiques de la diversité. La phylogénie étant donnée, trois vecteurs d'abondance (P1 à P3) sont de moins en moins réguliers : dans les cas C et D, la régularité maximale n'est pas obtenue pour des effectifs identiques, mais en augmentant les effectifs des espèces originales. ", echo=FALSE}
...
Dans un arbre phylogénétique (Figure&nbsp;\@ref(Pavoine2011), @Pavoine2011), la régularité de l'arbre [@Mooers1997] est un premier critère, complété éventuellement par les abondances. 
...
```{r Schleuter2010, fig.cap="Fonction d'appartenance de l'espace des niches de Schleuter et al., en une dimension. $x$, en ordonnée, est la valeur du trait considéré. Chaque espèce est considérée comme un ensemble flou dans l'espace des traits. Quatre espèces sont représentées, avec leur fonction d'appartenance $f_s(x)$. Le volume de l'espace des traits occupé, $\\mathit{FR}_{Im}$, est obtenu en intégrant les fonctions d'appartenance: c'est la zone grisée de la figure.", echo=FALSE}
...
Chaque espèce est supposée occuper un espace autour de sa position moyenne dans l'espace des traits, avec une fonction d'appartenance [@Zadeh1965] gaussienne multidimensionnelle (une représentation unidimensionnelle se trouve en figure&nbsp;\@ref(Schleuter2010), @Schleuter2010): la fonction d'appartenance, issue de la logique floue, peut être vue comme une densité de probabilité non normalisée.
...
L'indice d'équitabilité de @Mouillot2005a, que les auteurs nomment "indice de régularité fonctionnelle", est inspiré de l'indice d'équitabilité de Bulla (page~\@ref(eq:Bulla)).
...
Cette valeur est normalisée: $\mathit{PEW}_{s}=\frac{\mathit{EW}_{s}}{\sum_s{\mathit{EW}_{s}}}$.
Sa valeur de $\mathit{PEW}_{s}$ attendue pour le maximum d'équitabilité est $\frac{1}{(S-1)}$.
...
@Laliberte2010 généralisent $\mathit{FD}_{m}$ en proposant l'usage de n'importe quelle dissimilarité entre espèces, obtenue à partir de la méthode de Gower (vue page \@ref(Gower)), autorisant des variables qualitatives et des données manquantes, au-delà de la seule distance euclidienne entre traits quantitatifs.
...
La littérature de la diversité phylogénétique s'est intéressée tôt à l'originalité taxonomique des espèces parce que les questions traitées concernaient la conservation (intéressée par la valeur de l'héritage évolutif [@Faith2008]).
...
Dans l'arbre de la figure&nbsp;\@ref(ArbreA4), les valeurs de TD non normalisées sont $\frac{1}{2}$ pour les deux premières espèces et $\frac{1}{3}$ pour les trois autres: la racine de l'arbre est comptabilisée dans le nombre de noeuds.
Les valeurs de $\mathit{TD}_s$ sont respectivement $\frac{1}{4}$ et $\frac{1}{6}$ après normalisation.

```{r ArbreA4, fig.cap="Arbre phylogénétique ou fonctionnel hypothétique. L'arbre comprend 5 espèces dont les probabilités sont notées $p_s$, 3 périodes de durées $T_k$ délimitées par les noeuds. Les branches sont notées $b$ et indicées par la période à laquelle elles se terminent et un numéro d'ordre. La légende complète se trouve en figure&nbsp;\\@ref(Arbre).", echo=FALSE}
knitr::include_graphics('images/ArbreA.png')
```
...
L'originalité taxonomique est définie comme la distance phylogénétique moyenne entre l'espèce $s$ et les autres: $w_s=\frac{\sum_s{d_{s,t}}}{S-1}$.
...
$\frac{[1-(1-p_s)^n]}{{\mathbb E}(\xq{{S}}{}{n}{}{})}$ est la contribution de l'espèce $s$ à l'espérance du nombre d'espèces, comprise entre 0 et 1. 
...
Ce dernier atteint $\frac{1}{{\mathbb E}(\xq{{S}}{}{n}{}{})}$ (une espèce est échantillonnée à coup sûr dès que l'échantillon est assez grand) d'autant plus rapidement que $p_s$ est grand.
...
```{r ArbreA3, fig.cap="Arbre phylogénétique ou fonctionnel hypothétique. Rappel de la figure \\@ref(fig:ArbreA)(a).", echo=FALSE}
knitr::include_graphics('images/ArbreA.png')
```
...
La divergence de l'espèce 1 de la Figure&nbsp;\@ref(Arbre), page~\@ref(Arbre), est $T1+T2$, la longueur de la branche terminale, plus $\frac{T3}{2}$, parce que la branche est partagée par les espèces 1 et 2. 
La divergence de l'espèce 1, est donc $L_1 = T1+T2+\frac{T3}{2}$. 
La part de la divergence de l'espèce $s$ est $l_s = \frac{L_s}{FD}$.
...
Une meilleure définition [@Presley2014] de la taille de la niche est la somme des distances aux autres espèces: $t_s = \sum_t{d_{s,t}}$. La part de chaque espèce est $f_s=\frac{t_s}{\sum_t{t_t}}$ et la définition de la diversité fonctionnelle est
...
En se limitant à deux dimensions pour la lisibilité, la figure&nbsp;\@ref(ScheinerPsfsFig) présente les rectangles correspondant à la diversité d'abondance et fonctionnelle occupés par les espèces de la méta-communauté `Paracou618`, classées par fréquences décroissantes.

```{r ScheinerPsfsFig, echo=FALSE, results='hide', fig.width=6, fig.height=6, ref.label='ScheinerPsfsCode', fig.cap="Rectangles de surface fréquence $\\times$ taille de niche des espèces de la méta-communauté `Paracou618`. La diversité de Scheiner $^{q}\\!D(AF)$ est la diversité de leur surface."}
```
...
```{r ProfilDqAFFig, echo=FALSE, results='hide', ref.label='ProfilDqAFCode', fig.cap="Profil de diversité d'abondance et fonctionnelle de Scheiner de la méta-communauté `Paracou618`. Pointillés longs : diversité d'abondance; pointillés courts : diversité fonctionnelle; trait plein : diversité d'abondance et fonctionnelle."}
```
...
Si $q$ est grand, les espèces dont le produit des dimensions de la diversité est petit (que l'espèce soit rare, ait une divergence phylogénétique faible ou une niche étroite) sont négligées, alors que $q$ n'affecte que la fréquence dans le cadre de la phylodiversité $^{q}\!\bar{D}$ ou la banalité pour $\xq{D}{q}{\mathbf{Z}}{}{}$.
...
```{r Arbre, fig.cap="Arbre phylogénétique ou fonctionnel hypothétique. (a) Arbre complet. 5 espèces sont présentes ($S=5$). Une période de l'arbre est définie entre deux noeuds successifs: l'arbre contient $K=3$ périodes. Les hauteurs des périodes sont notées $T_k$. À chaque période correspond un arbre plus simple (b pour la période 2, c pour la période 3) dans lequel les espèces originales sont regroupées. Le nombre de feuilles de ces arbres est noté $L_k$. Les probabilités pour un individu d'appartenir à une feuille sont notées $u_{k,l}$.", echo=FALSE}
...
L'inertie des points représentants les espèces, précisément la moyenne des carrés des distances entre les espèces et leur centre de gravité (les probabilités $\mathbf{p}$ des espèces constituent leur poids), est alors égale à l'entropie de Rao $H_{\mathbf{\Delta}}\left(\mathbf{p}\right)$, où la matrice de distances $\mathbf{\Delta}$ est $\dfrac{\mathbf{D}^{\circ2}}{2}$: les valeurs de  $\mathbf{\Delta}$ valent la moitié du carré de celles de $\mathbf{D}$.
Cette représentation est étendue dans la double analyse en composantes principales de @Pavoine2004 (page~\@ref(sec:RaoDisc)).
...
```{r PCAFFig, echo=FALSE, results='hide', out.width='\\textwidth', ref.label='PCAFCode', fig.cap="Analyse en composante principale des traits foliaires"}
```
...
```{r HclustFig, echo=FALSE, results='hide', out.width='\\textwidth', ref.label='HclustCode', fig.cap="Arbre phylogénétique issu de la classification automatique."}
```
...
```{r PhylogRaoFig, echo=FALSE, results='hide', out.width='\\textwidth', ref.label='PhylogRaoCode', fig.cap="Présentation de l'arbre phylogénétique avec la contribution de chacune des variables."}
```
...
```{r originalityFig, echo=FALSE, results='hide', out.width='\\textwidth', ref.label='originalityCode', fig.cap="Originalité des espèces."}
```
...
@Guiasu2011, @Guiasu2012 proposent l'indice $\mathit{{GS}_{D}}$ pour ses propriétés mathématiques et fournissent sa décomposition:
...
```{r ArbreA2, fig.cap="Arbre phylogénétique ou fonctionnel hypothétique. Rappel de la figure \\@ref(fig:ArbreA)(a).", echo=FALSE}
knitr::include_graphics('images/ArbreA.png')
```
...
```{r ArbreA5, fig.cap="Arbre phylogénétique ou fonctionnel hypothétique. Rappel de la figure \\@ref(fig:ArbreA)(a). 5 espèces sont présentes ($S=5$), leurs probabilités notées $p_1$ à $p_5$. Les noms des branches sont affichés.", echo=FALSE}
knitr::include_graphics('images/ArbreA.png')
```
...
La somme est sur l'ensemble des branches de l'arbre, défini de façon identique à celle de @Allen2009, page \@ref(sec:HpI1).
...
```{r Chao2010, fig.cap="Arbre phylogénétique hypothétique non ultramétrique", echo=FALSE}
knitr::include_graphics('images/Chao2010.png')
```

@Chao2010 définissent la diversité phylogénétique selon l'équation (\@ref(eq:Dbarq)) quelle que soit la forme de l'arbre, y compris s'il n'est pas ultramétrique. 
...
Àpartir de la théorie mathématique des catégories [@Leinster2013], @Leinster2012 ont proposé une mesure de diversité applicable à une communauté dont la position des espèces dans un espace multidimensionnel euclidien est connue.
...
\begin{equation}
  \xq{D}{q}{\mathbf{Z}}{}{}={\left(\sum_s{p_s{\left(\mathbf{Zp}\right)}^{q-1}_s}\right)}^{\frac{1}{1-q}}.
  \label{eq:Dqz}
\end{equation}

$\xq{D}{q}{\mathbf{Z}}{}{}$ converge vers $\xq{D}{1}{\mathbf{Z}}{}{}$ quand $q$ tend vers 1:

\begin{equation}
  \xq{D}{1}{\mathbf{Z}}{}{}=\frac{1}{\prod_s{{\left(\mathbf{Zp}\right)}^{p_s}_s}}.
\end{equation}


## Diversité neutre

Leinster et Cobbold montrent que la diversité HCDT est un cas particulier de $\xq{D}{q}{\mathbf{Z}}{}{}$, pour la matrice identité:
...
L'exemple suivant utilise les données du calcul d'originalité, page~\@ref(MaxTheorique). 
...
Dans le cas particulier $q=2$, $\xq{D}{2}{\mathbf{Z}}{}{}$ est égal à $^{2}\!\bar{D}(T)$:
...
Enfin, l'entropie HCDT peut généraliser l'entropie de Shannon pour définir une mesure $Q_{\alpha}$ que nous noterons $\xq{H}{q}{\mathbf{Z}}{}{}$ pour la cohérence de nos notations ($q=\alpha$):

\begin{equation}
  \xq{H}{q}{\mathbf{Z}}{}{} = \sum_s{p_{s}\ln_q{\frac{1}{\left(\mathbf{Zp}\right)_s}}}.
\end{equation}
...
$\xq{H}{q}{\mathbf{Z}}{}{}$ est donc bien une entropie. 
...
Le logarithme d'ordre $q$ de $\xq{D}{q}{\mathbf{Z}}{}{}$ est $\xq{H}{q}{\mathbf{Z}}{}{}$:

\begin{equation}
  \ln_q{\xq{D}{q}{\mathbf{Z}}{}{}} = \xq[\,]{H}{q}{\mathbf{Z}}{}{}.
\end{equation}
...
```{r LineaireZpsqFig, fig.cap = "Approximation de ${(\\mathbf{Zp})}_{s}^{q-1}$ par son développement limité au premier ordre pour $q=1.5$ en haut ($r=q-1$) et $q=0.5$ en bas", fig.ncol = 2, echo = FALSE, ref.label='LineaireZpsqCode1'}

```

Le problème majeur de la diversité $\xq{D}{q}{\mathbf{Z}}{}{}$ est qu'elle fournit souvent des valeurs presque indépendantes de $q$ quand on l'applique à une matrice de similarité fonctionnelle [@Chiu2014b]. 
...
```{r EchelleFig, echo=FALSE, results='hide', out.width='\\textwidth', ref.label='EchelleCode', fig.cap="Transformation des distances (en abscisse) en similarités (en ordonnées) selon la valeur de $u$. Trait plein: $u=1$; pointillés longs : $u=2$; pointillés : $u=10$. Plus $u$ est grand, plus les similarités sont tassées vers 0."}
```
...
Comme les autres mesures de diversité, $\xq{D}{q}{\mathbf{Z}}{}{}$ est sujette au biais d'estimation. 
Deux estimateurs réduisant le biais existent [@Marcon2014e]. 
Le premier utilise la technique de @Chao2003 déjà vue pour la correction du biais de l'entropie de Shannon (page~\@ref(eq:ChaoShen)). 
L'estimateur de $\xq{H}{q}{\mathbf{Z}}{}{}$ est

\begin{equation}
  \xq{\tilde{H}}{q}{\mathbf{Z}}{}{}
...
L'estimateur de la diversité est $\xq{\tilde{D}}{q}{\mathbf{Z}}{}{}=e^{\xq{\tilde{H}}{q}{\mathbf{Z}}{}{}}_q$. 
...
L'estimateur de $\xq{D}{q}{\mathbf{Z}}{}{}$ est, pour $q\ne 1$,

\begin{equation}
  \xq{\tilde{D}}{q}{\mathbf{Z}}{}{}
  = {\left\{\hat{K}+\hat{V}
...
```{r ArbreA6, fig.cap="Arbre phylogénétique ou fonctionnel hypothétique. Rappel de la figure \\@ref(fig:ArbreA)(a). 5 espèces sont présentes ($S=5$), leurs probabilités notées $p_1$ à $p_5$. Les noms des branches sont affichés.", echo=FALSE}
knitr::include_graphics('images/ArbreA.png')
```

La diversité phylogénétique $^{q}\!\bar{D}(T)$ appliquée à un arbre ultramétrique est un cas particulier de $\xq{D}{q}{\mathbf{Z}}{}{}$ pour une matrice $\mathbf{Z}$ dont les lignes et colonnes sont la similarité des "espèces historiques", c'est-à-dire les couples (espèce actuelle; branche ancestrale)[@Leinster2012, proposition A7].


La matrice de similarité correspondant à l'arbre phylogénétique de la figure&nbsp;\@ref(figArbreA6) est la suivante.
...
Cependant, le calcul de $\xq{\hat{D}}{q}{\mathbf{Z}}{}{}$ à partir d'une matrice $\mathbf{Z}$ qui serait la simple transformation des distances de l'arbre phylogénétique, c'est-à-dire 1 moins les distances normalisées par la hauteur de l'arbre, ne donne pas la valeur de la diversité phylogénétique $^{q}\!\bar{D}(T)$, sauf dans le cas particulier de la diversité de Rao, $q=2$.

Pour un arbre non ultramétrique, il est possible de définir une matrice $\mathbf{Z}$ similaire à celle de la figure&nbsp;\@ref(MatricePhylo), mais ses valeurs non nulles ne sont pas égales à 1.
...
La diversité $\xq{D}{q}{\mathbf{Z}}{}{}$ et tous ses cas particuliers (phylodiversité et diversité neutre) peuvent être envisagés au niveau individuel plutôt qu'au niveau de l'espèce.
...
L'opération peut être répétée jusqu'à la désagrégation complète de la matrice où chaque ligne correspondrait à un individu et les probabilités seraient égales à $\frac{1}{N}$. 
...
Dans le cas le plus simple où $p_{s'}=p_{s""}$, la similarité intraspécifique est $\frac{(1+z_{s',s""})}{2}$. 
...
Pour mesurer la diversité neutre, les espèces sont placées dans un espace multidimensionnel, de dimension $S$. Chaque espèce est représentée par un vecteur de longueur $p_s$ sur son axe. Cette représentation a été utilisée par @Campos2009 qui ont relié la diversité de Simpson au volume de la sphère sur laquelle se trouve le point définissant la communauté (Figure&nbsp;\@ref(fig:Campos2009)).

```{r Campos2009, fig.cap="Représentation d'une communauté dans un espace multidimensionnel dont chaque axe correspond à une espèce. La communauté est composée de trois espèces. Le point représentant la communauté se trouve sur la sphère de rayon $r=\\sqrt{\\sum_s{p_s^2}}$. Le plan (qui est en réalité un hyperplan de dimension $S-1$) d'équation $\\sum_s{p_s}=1$ est représenté par un triangle.", echo=FALSE}
knitr::include_graphics('images/Campos2009.png')
```

La matrice de similarité entre les espèces $\mathbf{Z}$ permet d'affiner la représentation. Soit la matrice $\mathbf{C}=\sqrt{\mathbf{Z}}$.
...
Il est possible de calculer les $S$ valeurs propres notées $\lambda_s$ de la matrice des coordonnées des espèces et de les normaliser: $\mu_s=\frac{\lambda_s}{\sum_s{\lambda_s}}$.
...
La diversité HCDT des valeurs propres est la diversité de la communauté, qui sera notée ici $\xq{D}{q}{\mathbf{Z}}{}{}(\Lambda)$.
...
Si $\mathbf{C}$ est la racine carrée d'une matrice de similarité au sens strict $\mathbf{Z}$, c'est-à-dire contenant des valeurs comprises entre 0 et 1 et dont les éléments de la diagonale sont tous égaux à 1, alors $\xq{D}{2}{\mathbf{Z}}{}{}(\Lambda)$ est égale à la diversité de Rao appliquée à une matrice de distances $\mathbf{D}=1-\mathbf{Z}$.
...
Si $\mathbf{Z}=\mathbf{I}_s$, l'ACP ne modifie pas le nuage de points original et $\xq{D}{q}{\mathbf{Z}}{}{}(\Lambda)=^{q}\!D$.
...
```{r DqZLambdaFig, echo=FALSE, results='hide', ref.label='DqZLambdaCode', fig.cap="Profil de diversité des valeurs propres de la méta-communauté `Paracou618`."}
```
...
$\xq{D}{q}{\mathbf{Z}}{}{}$ est l'exponentielle d'ordre $q$ de l'entropie de Ricotta et Szeidl, mais aussi l'inverse de la banalité moyenne des espèces de la communauté.

$\xq{D}{q}{\mathbf{Z}}{}{}$ et $^{q}\!D$ sont des nombres effectifs d'espèces, respectent toutes les propriétés demandées à une mesure de diversité dont le principe de réplication. 
Leurs valeurs sont identiques pour $q=2$ (diversité de Rao). 
$\xq{D}{q}{\mathbf{Z}}{}{}$ est moins sensible au paramètre $q$ que $^{q}\!D(T)$: la Figure&nbsp;\@ref(ComparaisonDqFig) compare les deux mesures.

Le rôle du paramètre est assez différent: dans $\xq{D}{q}{\mathbf{Z}}{}{}$, la banalité des espèces est à la puissance $q$ alors que leur probabilité est à la puissance 1.
...
```{r ComparaisonDqFig, echo=FALSE, results='hide', ref.label='ComparaisonDqCode', fig.cap="Diversité phylogénétique (l'arbre est une taxonomie) d'ordre $q$ de deux hectares de forêt de Paracou: $\\xq{D}{q}{\\mathbf{Z}}{}{}$ (trait plein) et $^{q}\\!\\hat{D}(T)$ (pointillé)."}
```
...
$\xq{D}{q}{\mathbf{Z}}{}{}$ est particulièrement intéressante pour mesurer la diversité fonctionnelle, souvent définie à partir d'une matrice de distances entre les espèces. 
