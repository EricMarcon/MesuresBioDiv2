...
```{r DistributionsFig, echo=FALSE, results='hide', message=FALSE, warning=FALSE, ref.label='DistributionsCode',  fig.cap="(ref:DistributionsFig)"}
```

Code de la figure \@ref(fig:DistributionsFig):
...
La distribution est aussi le résultat d'un algorithme de bâton brisé (*broken stick*) séquentiel:
...
fit_volkov@min
```
Les paramètres du modèle de communauté locale de la théorie neutre sont $\theta$, le "nombre fondamental" égal à deux fois le nombre d'espèces apparaissant par pas de temps dans la méta-communauté, $m$, le taux de migration, et $J$, la taille de la communauté locale.

Graphiquement, l'ajustement des deux modèles est très proche.
...
Selon le critère d'information d'Akaike [AIC: @Akaike1998], l'ajustement est meilleur mais la différence est petite et la complexité du modèle et des calculs pour l'estimer ne se justifient pas en général: le modèle de Volkov est très peu utilisé en pratique.
